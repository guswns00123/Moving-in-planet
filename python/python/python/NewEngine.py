"""
/∗
∗ CSCI3180 Principles of Programming Languages
∗
∗ --- Declaration ---
∗
∗ I declare that the assignment here submitted is original except for source
∗ material explicitly acknowledged. I also acknowledge that I am aware of
∗ University policy and regulations on honesty in academic work, and of the
∗ disciplinary guidelines and procedures applicable to breaches of such policy
∗ and regulations, as contained in the website
∗ http://www.cuhk.edu.hk/policy/academichonesty/
∗
∗ Assignment 2
∗ Name : Yoo Hyun Jun
∗ Student ID : 1155100531
∗ Email Addr : hjyoo8@cse.cuhk.edu.hk
∗/
"""
from Map import Map
from Cell import Plain, Mountain, Swamp
from GameCharacter import Player, Goblin
from Trap import Trap 
from Volcano import Volcano
class NewEngine:
    def __init__(self, data_file):
        self._data_file = data_file
        self._actors = []
        self._remove = [] 
        self._map = None 
        self._player = None 
        with open(data_file, "r") as fp:
            line = fp.readline()
            if not line:
                return None
            else:
                items = line.split()
                if len(items) != 7:
                    print("INVALID DATA FILE.")
                    return None
                num_of_row = int(items[0])
                num_of_col = int(items[1])
                p_ox = int(items[2])
                p_hp = int(items[3])
                num_of_goblins = int(items[4])
                num_of_traps = int(items[5])
                num_of_volcanoes = int(items[6])

            self._map = Map(num_of_row, num_of_col)
            
            # TODO: initialize each cell of the map object 
            #       using the build_cell method
            for i in range(num_of_row):
                line  = fp.readline()
                if not line:
                    return None
                else:
                    items = line.split()
                    if len(items) != num_of_col:
                        print("INVALID DATA FILE")
                        return None
                    for j in range(num_of_col):
                        if items[j] == 'P':
                            self._map.build_cell(i,j,Plain(i,j))
                        elif items[j] == 'S':
                            self._map.build_cell(i,j,Swamp(i,j))
                        elif items[j] == 'M':
                            self._map.build_cell(i,j,Mountain(i,j))


            # END TODO
            
            self._player = Player(num_of_row - 1, 0, p_hp, p_ox)
            init_cell = self._map.get_cell(num_of_row - 1, 0) 
            self._player.set_occupying(init_cell)
            init_cell.set_occupant(self._player)
            self._actors.append(self._player)
            # TODO: initilize the position of the player 
            #       using the set_occupant and set_occupying method;
            #       add the player to _actors array

            for gno in range(num_of_goblins):
                # TODO: initilize each Goblin on the map
                #       using the set_occupant and set_occupying method;
                #       add each Goblin to _actors array 
                line = fp.readline()
                if not line:
                    return None
                else:
                    items = line.split()
                    pos_row = int(items[0])
                    pos_col = int(items[1])
                    actions = items[2:]
                    gob = Goblin(pos_row, pos_col, actions)
                    self._actors.append(gob)
                    init_cell = self._map.get_cell(pos_row, pos_col)
                    gob.set_occupying(init_cell)
                    init_cell.set_occupant(gob)
                # END TODO 

            for tno in range(num_of_traps):
                # TODO: initilize each Trap on the map
                #       using the set_occupant and set_occupying method;
                line = fp.readline()
                items = line.split()
                pos_row = int(items[0])
                pos_col = int(items[1])
                trap = Trap(pos_row, pos_col)
                init_cell = self._map.get_cell(pos_row, pos_col)
                init_cell.set_occupant(trap)
                trap.set_occupying(init_cell)
                # END TODO 

            for vno in range(num_of_volcanoes):
                # TODO: initilize each Volcano of the map object 
                #       using the build_cell method 
                #       add each volcano to _actors array 
                line = fp.readline()
                items = line.split()
                pos_row = int(items[0])
                pos_col = int(items[1])
                freq = int(items[2])
                vol = Volcano(pos_row, pos_col, freq)
                self._actors.append(vol)
                self._map.build_cell(pos_row, pos_col, vol)

                # END TODO 

    def run(self):
        # main rountine of the game
        self.print_info()
        
        while not self.state():            
            for obj in self._actors:
            	#change
                if obj.get_active():
                	obj.act(self._map)
                
            self.print_info()
            self.clean_up()
        self.print_result()

    def clean_up(self):
        # TODO: 
        for obj in self._actors:
            if obj._active == False:
                self._actors.remove(obj)
                self._remove.append(obj)
        # END TODO 

    # check if the game ends and return if the player win or not.
    def state(self):
        # TODO: 
        if self._player.get_hp() <= 0 or self._player.get_oxygen() <= 0:
            return -1
        elif self._player.get_row() == 0 and self._player.get_col() == self._map.get_cols() -1:
            return 1
        else:
            return 0
        # END TODO 

    def print_info(self):
        self._map.display()
        surround_cells = self._map.get_neighbours(self._player.get_row(), self._player.get_col())
        trap_num = 0
        
        for i in range(len(surround_cells)):
            if surround_cells[i].occupant != None:
                if surround_cells[i].occupant.get_name() == "Trap":
                    trap_num +=1
        # TODO: display the remaining oxygen and HP
        print("Oxygen: {0}, HP: {1}, Trap: {2}".format(self._player._oxygen, self._player._hp, trap_num ))
        # END TODO

    def print_result(self):
        if self.state() == 1:
            print("\033[1;33;41mCongrats! You win!\033[0;0m")
        elif self.state() == -1:
            print("\033[1;33;41mBad Luck! You lose.\033[0;0m")

        
